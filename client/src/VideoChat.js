...
const VideoChat = () => {
  ...
  useEffect(() => {
    ...
    const runUser = async () => {
      ...
      let createPeerConnection = async () => {
        ...
        // Set up track event for remote streams
        peerConnection.ontrack = async (event) => {
          event.streams[0].getTracks().forEach((track) => {
            remoteStream.addTrack(track);
          });
        ...
        sendChannelRef.current = sendChannel; // Save the channel to the ref

        // Set up event handlers for sendChannel
        sendChannel.onopen = () => console.log("Data channel is open");
        sendChannel.onclose = () => console.log("Data channel is closed");

        // Set up event handler for receiving data channels
        peerConnection.ondatachannel = (event) => {
          console.log("Data channel received");
          receiveChannelRef.current = event.channel; // Save the channel to the ref
          receiveChannelRef.current.onmessage = onReceiveChannelMessageCallback; // Handle received messages
        };
      };
      ...
      // }
      function onReceiveChannelMessageCallback(event) {
        const receivedMessage = event.data; // Get the received data
        chatTextAreaRef.current.innerHTML += `
          <div style='margin-top:2px; margin-bottom:2px;'><b>Stranger: </b>${receivedMessage}</div>
        `;
      }
  ...
  const sendData = () => {
    ...
    if (
      ...
      sendChannelRef.current.send(msgData); // Send data through the data channel
    }

    chatTextAreaRef.current.innerHTML += `
      <div style='margin-top:2px; margin-bottom:2px;'><b>Me: </b>${msgData}</div>
    `;
    msgInputRef.current.value = ""; // Clear input field after sending
  };
